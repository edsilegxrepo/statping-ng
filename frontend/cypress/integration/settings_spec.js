...
context("Settings Tests", () => {
	...
	it("should update Statping settings", () => {
		...
		cy.get("#description")
			.clear()
			.type("Statping can use Cypress e2e testing to make it more stable!");
	...
	it("should confirm Statping settings", () => {
		...
		cy.get("#description").should(
			"have.value",
			"Statping can use Cypress e2e testing to make it more stable!",
		);
	...
	it("should create Local Assets", () => {
		...
		cy.get("#assets_dir").should(
			"contain",
			"github.com/statping-ng/statping-ng/assets",
		);
	});
	...
	it("should save Local Assets", () => {
		...
		cy.get(".CodeMirror textarea").type(
			"{downarrow}$example-variable: #bababa;{enter}",
			{
				force: true,
			},
		);
