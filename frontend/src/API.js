...
const _tokenKey = "statping_auth"; // Reserved for token-based auth implementation
...
class Api {
	...
	async service_hits(id, start, end, group, fill = true) {
		return axios
			.get(
				`api/services/${id}/hits_data?start=${start}&end=${end}&group=${group}&fill=${fill}`,
			)
			.then((response) => response.data);
	}
	...
	async service_ping(id, start, end, group, fill = true) {
		return axios
			.get(
				`api/services/${id}/ping_data?start=${start}&end=${end}&group=${group}&fill=${fill}`,
			)
			.then((response) => response.data);
	}
	...
	async service_failures(id, start, end, limit = 999, offset = 0) {
		return axios
			.get(
				`api/services/${id}/failures?start=${start}&end=${end}&limit=${limit}&offset=${offset}`,
			)
			.then((response) => response.data);
	}
	...
	async service_failures_count(id, start, end) {
		return axios
			.get(`api/services/${id}/failures_count?start=${start}&end=${end}`)
			.then((response) => response.data);
	}
	...
	async service_failures_data(id, start, end, group, fill = true) {
		return axios
			.get(
				`api/services/${id}/failure_data?start=${start}&end=${end}&group=${group}&fill=${fill}`,
			)
			.then((response) => response.data);
	}
	...
	async service_failures_delete(service_id) {
		return axios
			.delete(`api/services/${service_id}/failures`)
			.then((response) => response.data);
	}
	...
	async notifier_test(method, data) {
		return axios
			.post(`api/notifier/${method}/test`, data)
			.then((response) => response.data);
	}

	async notifier_logs(method, start, end) {
		return axios
			.get(`api/notifier/${method}/logs?start=${start}&end=${end}`)
			.then((response) => response.data);
	}

	async incident_update(data) {
		return axios
			.post(`api/incidents/${data.id}`, data)
			.then((response) => response.data);
	}
	...
	async incident_create_update(incident_id, data) {
		return axios
			.post(`api/incidents/${incident_id}/updates`, data)
			.then((response) => response.data);
	}
	...
	async checkin_create(data) {
		return axios
			.post(`api/services/${data.service_id}/checkins`, data)
			.then((response) => response.data);
	}

	async checkin_update(data) {
		return axios
			.post(`api/checkins/${data.id}`, data)
			.then((response) => response.data);
	}
	...
	async settings_import(data) {
		const params = new URLSearchParams(data);
		return axios
			.post("api/settings/import", params)
			.then((response) => response.data);
	}
