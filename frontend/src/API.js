...
class Api {
  ...
  async service_heatmap(id, start, end, group = '24h') {
  ...
  async check_token(token) {
    ...
    return axios.post('api/users/token', params).then((response) => response.data)
  }
  ...
  async github_release() {
    return fetch('https://api.github.com/repos/statping-ng/statping-ng/releases/latest').then((response) =>
      response.json()
    )
  }
}
