...
export default {
	...
	methods: {
		...
		async lastDaysFailures() {
			const start = this.beginningOf(
				"day",
				this.nowSubtract(86400 * this.days_to_show),
			);
			const end = this.endOf("today");
			// Call both endpoints to get both success and failure data for the past 90 days
			const failuresPromise = Api.service_failures_data(
				this.service.id,
				this.toUnix(start),
				this.toUnix(end),
				"24h",
			...
			const hitsPromise = Api.service_hits(
				this.service.id,
				this.toUnix(start),
				this.toUnix(end),
				"24h",
			...
			const [failuresData, hitsData] = await Promise.all([
				failuresPromise,
				hitsPromise,
			]);
		...
		mergeData(failuresData, hitsData) {
			...
			// Process hits data
			hitsData.forEach((d) => {
				let _date = this.parseISO(d.timeframe);
