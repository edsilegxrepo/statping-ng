		...
		return (fp) => {
			...
			var plugin = {
				...
				onDestroy: function onDestroy() {
					if (!config.input) secondInput.parentNode?.removeChild(secondInput);
				},
				onValueUpdate: function onValueUpdate(selDates) {
					...
					var _fp$selectedDates$map = fp.selectedDates.map((d) =>
						fp.formatDate(d, dateFormat),
					);

					var _fp$selectedDates$map2 = _fp$selectedDates$map[0];
					fp._input.value =
						_fp$selectedDates$map2 === void 0 ? "" : _fp$selectedDates$map2;
					var _fp$selectedDates$map3 = _fp$selectedDates$map[1];
					secondInput.value =
						_fp$selectedDates$map3 === void 0 ? "" : _fp$selectedDates$map3;
				},
			};
			return plugin;
		};
