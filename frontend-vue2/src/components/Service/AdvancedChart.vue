...
export default {
			...
			main_chart_options: {
				...
				tooltip: {
					...
					custom: ({ series, seriesIndex, dataPointIndex, w }) => {
						...
						let failText = failuresVal
							? `${failuresVal} Failures`
							: "0 Failures";
	...
	computed: {
		...
		params() {
			return {
				start: this.toUnix(new Date(this.start)),
				end: this.toUnix(new Date(this.end)),
			};
		},
	},
	watch: {
		start: function (_n, _o) {
			this.update_data();
		},
		end: function (_n, _o) {
			this.update_data();
		},
		group: function (_n, _o) {
			this.update_data();
		},
	},
	methods: {
		...
		async load_failures(
			...
			return await Api.service_failures_data(
				this.service.id,
				start,
				end,
				group,
