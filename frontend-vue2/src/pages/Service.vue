...
const MessageBlock = () =>
	import(/* webpackChunkName: "index" */ "@/components/Index/MessageBlock");
const ServiceFailures = () =>
	import(
		/* webpackChunkName: "service" */ "@/components/Service/ServiceFailures"
	);
const Checkin = () =>
	import(/* webpackChunkName: "dashboard" */ "@/forms/Checkin");
const ServiceHeatmap = () =>
	import(
		/* webpackChunkName: "service" */ "@/components/Service/ServiceHeatmap"
	);
...
const AdvancedChart = () =>
	import(
		/* webpackChunkName: "service" */ "@/components/Service/AdvancedChart"
	);
const DailyFailuresChart = () =>
	import(
		/* webpackChunkName: "service" */ "@/components/Service/DailyFailuresChart"
	);
...
export default {
		...
		return {
			...
			chartOptions: {
				...
				chart: {
					id: "mainchart",
					events: {
						dataPointSelection: (event, _chartContext, _config) => {
	...
	computed: {
		...
		params() {
			return {
				start: this.toUnix(new Date(this.start_time)),
				end: this.toUnix(new Date(this.end_time)),
			};
		},
		uptimeSeries() {
		...
		messagesInRange() {
			return this.$store.getters
				.serviceMessages(this.service.id)
				.filter((m) => this.inRange(m));
		},
	},
	created() {
		this.fetchData();
	...
	methods: {
		...
		async chartHits(_start = 0, _end = 99999999999) {
			this.data = await Api.service_hits(
				this.service.id,
				this.params.start,
				this.params.end,
				this.group,
		...
		async chartFailures(_start = 0, _end = 99999999999) {
			this.failures_data = await Api.service_failures_data(
				this.service.id,
				this.params.start,
				this.params.end,
				this.group,
